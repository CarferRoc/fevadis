...
export const chatService = {
    ...
    async getOrCreateDirectChat(userId1: string, userId2: string) {
        ...
        if (existing && existing.length > 0) {
            const direct = existing.find((c: Chat) => c.participants.length === 2);
    ...
    async createGroupChat(name: string, participants: string[], adminId: string) {
        if (!participants.includes(adminId)) participants.push(adminId);
        const { data, error } = await supabase
            .from('chats')
            .insert({
                participants,
    ...
    subscribeToMessages(chatId: string, onMessage: (msg: Message) => void): RealtimeChannel {
        return supabase
